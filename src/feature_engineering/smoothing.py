...
class RollingMaxCausal:
    def __init__(self, win_len):
        self.position_value_list = [(-float('inf'), -float("inf"))]
...
class ExponentialSmoother:
    ...
    def get_next(self, value, delta_t):
        a = - np.expm1(-delta_t / self.time_to_divide_by_e)
        self.num = (1.0 - a) * self.num  + a * value #* delta_t
        self.den = (1.0 - a) * self.den + a #* delta_t
        return self.num / self.den

    def __call__(self, value, delta_t):
        return self.get_next(value, delta_t)
